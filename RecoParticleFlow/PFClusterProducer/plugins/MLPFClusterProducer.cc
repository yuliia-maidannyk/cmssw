...
void MLPFClusterProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (int n = 0; n < numClusters; ++n) {
      for (int i = 0; i < maxClusters; ++i) {
          ...
          centers[n][i] = {
              cx + indices[n][i](0, 0) - cropSize / 2.0f, // + cropSize / 2.0f,
  ...
  for (int n = 0; n < numClusters; ++n) {
      for (int i = 0; i < maxClusters; ++i) {
          ...
          float cy = centers[n][i].second;
          float si = seeds[n][i];
          float eni = energies[n][i];
          if (si <= outputThreshold) continue;
          int col  = static_cast<int>(std::round(cy));
          int iphi = static_cast<int>(std::round(cx));
          int ieta = (col < 85) ? (col - 85) : (col - 84);
          std::cout << "DEBUG Cluster " << n << "," << i 
                    << ": seed=" << si 
                    << ", energy=" << eni 
                    << ", center=(ieta=" << ieta << ", iphi=" << iphi << ")" 
                    << std::endl;
          ...
          validClusters.push_back({eni, -1.0f,
              math::XYZPoint(pos.x(), pos.y(), pos.z()),
              ebId});
      }
  }
